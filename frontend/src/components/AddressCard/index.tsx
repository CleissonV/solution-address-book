...
export function AddressCard({
  ...
  async function setPrimary() {
    setError('');
    try {
      await primaryMutation.mutateAsync(address.id);
      setMenuOpen(false);
    } catch (exception) {
  ...
  return (
    <article
      className={cn(styles.card, address.primary && styles.primary)}
    ...
    >
          ...
          <div className={styles.menuWrap}>
            ...
            {menuOpen && (
              <div className={styles.menu}>
                ...
                {!address.primary && (
                  <button onClick={setPrimary} disabled={primaryMutation.isPending}>
                    <Star size={16} />
                    Tornar principal
                  </button>
                )}
      ...
      {!readOnly ? (
        <AddressDialog userId={userId} address={address} open={editing} onOpenChange={setEditing} />
      ) : null}
      {!readOnly ? (
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              <div>
                ...
                <AlertDialogDescription>
                  {address.street}, {address.number} será removido permanentemente. Esta ação não
                  pode ser desfeita.
                </AlertDialogDescription>
              </div>
            </AlertDialogHeader>

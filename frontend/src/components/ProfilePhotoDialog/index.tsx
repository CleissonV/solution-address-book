      ...
      >
        <form id="profile-photo" className={styles.form} onSubmit={submit}>
          ...
          <div className={styles.preview}>
            {previewUrl ? (
              <img src={previewUrl} alt="Pré-visualização da nova foto" />
            ) : (
              <ProfileAvatar
                user={{
                  ...user,
                  profilePhotoVersion: removeCurrent ? undefined : user.profilePhotoVersion,
                }}
                size="xl"
              />
            )}
            <div>
              <strong>{file ? file.name : user.name}</strong>
              <span>
                {file
                  ? 'Nova foto selecionada'
                  : removeCurrent
                    ? 'Foto atual será removida'
                    : 'Foto atual'}
              </span>
            </div>
          </div>
          ...
          {user.profilePhotoVersion !== undefined && !file ? (
            <Button
              ...
              variant="danger"
              size="small"
              onClick={() => setRemoveCurrent(!removeCurrent)}
            >
              <Trash2 size={16} />
              {removeCurrent ? 'Manter foto atual' : 'Remover foto atual'}
            </Button>
          ) : (
            <p className={styles.hint}>
              <ImagePlus size={16} />A imagem será recortada visualmente para caber no avatar.
            </p>
          )}
        </form>
      </Dialog>

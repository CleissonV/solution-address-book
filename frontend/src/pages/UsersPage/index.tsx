    ...
    <div className={styles.page}>
      <header className={styles.header}>
        <div>
          ...
          <h1>Usuários</h1>
          <p>Gerencie cadastros e endereços da plataforma.</p>
        </div>
        <CreateUserDialog />
      </header>
      <section className={styles.stats}>
        ...
        <article className={styles.statCard}>
          ...
          <div>
            ...
            <strong>{data.length - activeUsers}</strong>
          </div>
        </article>
      </section>
      <section className={styles.content}>
        ...
        {isLoading ? (
          ...
          <Alert className={styles.loadError}>Não foi possível carregar os usuários.</Alert>
        ) : filtered.length === 0 ? (
          <div className={styles.empty}>
            <UsersRound />
          ...
          <div className={styles.list}>
            {filtered.map((item) => (
              <button
                key={item.id}
                className={cn(styles.row, item.status === 'INACTIVE' && styles.inactive)}
                onClick={() => navigate(`/users/${item.id}`)}
              >
                <ProfileAvatar
                  user={item}
                  size="large"
                  className={item.status === 'INACTIVE' ? styles.inactiveAvatar : undefined}
                />
                <span className={styles.main}>
                  <strong>{item.name}</strong>
                  <small>{formatCpf(item.cpf)}</small>
                </span>
                <span className={styles.badges}>
                  <Badge tone={item.role === 'ADMIN' ? 'brand' : 'neutral'}>
                    {item.role === 'ADMIN' ? 'Administrador' : 'Usuário'}
                  </Badge>
                  {item.status === 'INACTIVE' ? <Badge tone="danger">Inativo</Badge> : null}
                </span>

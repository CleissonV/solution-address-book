...
export function useCreateUser() {
  ...
  return useMutation({
    mutationFn: async (input: CreateUserInput) => (await api.post<UserSummary>('/users', input)).data,
...
export function useUpdateUser(userId: string) {
  const client = useQueryClient()
  return useMutation({
    mutationFn: async (input: UpdateUserInput) => (await api.put<UserDetails>(`/users/${userId}`, input)).data,
    onSuccess: (updated) => {
...
export function useUpdateUserStatus(userId: string) {
  ...
  return useMutation({
    mutationFn: async (status: UserStatus) => (
      await api.patch<UserDetails>(`/users/${userId}/status`, { status })
    ).data,
    onSuccess: (updated) => {
      client.setQueryData(['users', userId], updated)
      client.setQueryData<UserSummary[]>(['users'], (current) => current?.map((user) => (
        user.id === userId ? { ...user, status: updated.status } : user
      )))
    },
  })
}
...
export function useProfilePhoto(userId: string, version?: number) {
  return useQuery({
    ...
    queryFn: async () => (await api.get<Blob>(`/users/${userId}/photo`, { responseType: 'blob' })).data,
...
function updatePhotoVersion(client: QueryClient, userId: string, version?: number) {
  ...
  client.setQueryData<UserSummary[]>(['users'], (current) => current?.map((user) => (
    user.id === userId ? { ...user, profilePhotoVersion: version } : user
  )))
}
...
export function useCreateAddress(userId: string) {
  const client = useQueryClient()
  return useMutation({
    mutationFn: async (input: AddressInput) => (await api.post<Address>(`/users/${userId}/addresses`, input)).data,
...
export function useUpdateAddress(userId: string, addressId: string) {
  const client = useQueryClient()
  return useMutation({
    mutationFn: async (input: AddressInput) => (await api.put<Address>(`/users/${userId}/addresses/${addressId}`, input)).data,
    onSuccess: () => client.invalidateQueries({ queryKey: ['users', userId] }),
  })
}
...
export function useSetPrimary(userId: string) {
  const client = useQueryClient()
  return useMutation({
    mutationFn: async (addressId: string) => (await api.patch<Address>(`/users/${userId}/addresses/${addressId}/primary`)).data,
    onSuccess: () => client.invalidateQueries({ queryKey: ['users', userId] }),
  })
}

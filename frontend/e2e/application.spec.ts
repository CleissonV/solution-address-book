...
const profilePng = Buffer.from(
  'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Wl2nqAAAAAASUVORK5CYII=',
  'base64',
);
...
test('executa jornada de usuário e endereço com autorização por perfil', async ({ page }) => {
  ...
  const roleEscalationStatus = await page.evaluate(
    async ({ currentUserUrl, name, cpf }) => {
  ...
  await page.getByRole('button', { name: 'Excluir', exact: true }).click();
  await page
    .getByRole('alertdialog', { name: 'Excluir endereço?' })

...
public class ProfilePhotoService {
    ...
    public ProfilePhotoService(
            ProfilePhotoRepository photoRepository, UserService userService, Clock clock) {
    ...
    public ProfilePhotoResponse upload(UUID userId, MultipartFile file, AuthenticatedUser actor) {
        ...
        long previousVersion =
                user.getProfilePhotoVersion() == null ? 0 : user.getProfilePhotoVersion();
        long version = Math.max(now.toEpochMilli(), previousVersion + 1);

        ProfilePhotoEntity photo =
                photoRepository
                        .findById(userId)
    ...
    public void delete(UUID userId, AuthenticatedUser actor) {
        ...
        if (!photoRepository.existsById(userId)) {
            throw new DomainException("PROFILE_PHOTO_NOT_FOUND", "Foto de perfil nao encontrada.");
        }
    ...
    private static byte[] readAndValidate(MultipartFile file) {
        if (file.isEmpty()) {
            throw new DomainException("EMPTY_PROFILE_PHOTO", "Selecione uma foto.");
        }
        if (file.getSize() > MAX_PHOTO_SIZE) {
            throw new DomainException("PHOTO_TOO_LARGE", "A foto deve ter no maximo 2 MB.");
        }
        try {
            return file.getBytes();
        } catch (IOException exception) {
            throw new DomainException("PHOTO_READ_ERROR", "Nao foi possivel ler a foto.");
        }
    }
    ...
    static String detectContentType(byte[] content) {
        ...
        throw new DomainException("INVALID_PHOTO_TYPE", "Use uma imagem PNG ou JPEG.");
    }

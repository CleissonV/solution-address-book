...
public class UserService {
    ...
    public UserDetailsResponse get(UUID requestedId, AuthenticatedUser actor) {
        ...
        return userRepository
                .findWithAddressesById(requestedId)
    ...
    @Transactional
    public UserDetailsResponse update(
            UUID requestedId, UpdateUserRequest request, AuthenticatedUser actor) {
        assertCanAccess(actor, requestedId);
        List<UserEntity> administrators =
                "ADMIN".equals(actor.role()) && request.role() != null
                        ? userRepository.findAllByRoleForUpdate(UserRole.ADMIN)
                        : List.of();
        UserEntity user =
                userRepository.findByIdForUpdate(requestedId).orElseThrow(UserService::notFound);
        assertActive(user);
        String cpf = Cpf.normalize(request.cpf());
        if (!Cpf.isValid(cpf)) {
            throw new DomainException("INVALID_CPF", "CPF invalido.");
        ...
        if (!admin && request.role() != null) {
            throw new DomainException(
                    "ROLE_CHANGE_FORBIDDEN", "Usuario comum nao pode alterar nivel de acesso.");
        }
    ...
    @Transactional
    @PreAuthorize("hasRole('ADMIN')")
    public UserDetailsResponse updateStatus(
            UUID requestedId, UpdateUserStatusRequest request, AuthenticatedUser actor) {
        if (request.status() == UserStatus.INACTIVE && actor.id().equals(requestedId)) {
            throw new DomainException(
                    "SELF_DEACTIVATION_FORBIDDEN", "Voce nao pode desativar a propria conta.");
        }
    ...
    private void assertCanAccess(AuthenticatedUser actor, UUID requestedId) {
        if (!"ADMIN".equals(actor.role()) && !actor.id().equals(requestedId)) {
            throw new DomainException(
                    "ACCESS_DENIED", "Voce nao pode acessar dados de outro usuario.");
        }
    }
    ...
    private static void assertActive(UserEntity user) {
        if (!user.isActive()) {
            throw new DomainException(
                    "ACCOUNT_INACTIVE_READ_ONLY", "Reative a conta antes de alterar seus dados.");
        }
    }
